...
starting_points = list(c(google, microsoft, funger), c(microsoft-.01, google+.5, funger-.1), c(microsoft+.01, funger-.01, google+.005))
three_loc <- llply(starting_points, function(sp) optim(sp, p2lsN_cost_opt, control=list(trace=0)))
...
make_map(dc.maps[[2]]) + geom_polygon(data=datapoly, aes(x=x,y=y,color=value,group=id), alpha=.1, size=2) +
    scale_color_continuous(low="blue", high="red")
# one of these gets truncated by the plotter! 
