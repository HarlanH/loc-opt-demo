...
cat(sprintf("Point-to-point Funger cost: %0.3f", 
      p2p_cost_funger))
cat(sprintf("Point-to-point Microsoft cost: %0.3f",
      p2p_cost_microsoft))
cat(sprintf("Point-to-point Google cost: %0.3f",
      p2p_cost_google))
...
location was provided. Skip the somewhat slow surface-of-a-sphere
distance now, and just use a simpler version with a tangent plane.
...
initial conditions give the same result.
...
If there are multiple locations, define as cost as the _minimum_ cost, 
for each person, to get to any location.
...
The three past locations are substantially better in the 3-location cost 
function (`r tri1_cost`) than the 1-location optimum plus two other locations
(`r tri2_cost`)! Show that on a map:
...
to find 3 optimal locations. Now, using "L-BFGS-B" algorithm, which
is an iterative quasi-Newton algorithm that supports bounds on the
parameters. Plot triangles found in each run on top of each other. Uh oh.
...
gradient heuristics, to (likely) find the global optimum.
...
three_loc_de <- DEoptim(cmpfun(p2lsN_cost_opt), lower=lower_box, upper=upper_box,
                     control=list(NP=80, itermax=200, trace=FALSE, 
                                  strategy=2, c=.5, VTR=362, reltol=.0001, steptol=20))
...
* Change the cost function (`p2ls_cost_opt`) to be the mean of the two best 
  point-to-line-segment costs, instead of just the best. How does this change the 
...
* Is there a variant of the cost function in the 3-location problem that's 
  convex, allowing fast optimization?
