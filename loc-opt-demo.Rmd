...
We're defining a simple cost function based on the sums of the point-to-point distances between 
...
Note: The sum of convex cost functions is provably convex.
...
Interestingly, with point-to-line-segment (work location) cost, 
Google is very slightly better than Funger, 
whereas with point-to-point (commute) cost, Funger was substantially better. 
...
initial conditions give the same result. The individual cost functions
are not quite convex (all points on the line segment are optima), but it
may be that the sum, in practice, is, at least if you start in a reasonable
neighborhood.
...
(`r tri2_cost`)! Show that on a map (blue = good; red = less good):
