...
p2p_venue_costs$cost_color <- with(p2p_venue_costs, 9-round(8*sqrt((cost-min(cost))/(max(cost)-min(cost)))))
markers <- dlply(p2p_venue_costs, 1, function(row) with(row, mk_circle(lat, lon, cost_color)))
names(markers) <- NULL # so converter creates a list
map$geoJson(markers,
...
p2ls_venue_costs$cost_color <- with(p2ls_venue_costs, 
                                    9-round(8*sqrt((cost-min(cost))/(max(cost)-min(cost)))))
markers <- dlply(p2ls_venue_costs, 1, function(row) with(row, mk_circle(lat, lon, cost_color)))
names(markers) <- NULL # so converter creates a list
map$geoJson(markers,
    pointToLayer='#! function (feature, latlng) {
        return L.circleMarker(latlng, {
...
grd <- expand.grid(lat=seq(from=times_square[[1]]-.25, to=times_square[[1]]+.25, length.out=41),
                        lon=seq(from=times_square[[2]]-.25, to=times_square[[2]]+.25, length.out=41))
grd <- adply(grd, 1, 
