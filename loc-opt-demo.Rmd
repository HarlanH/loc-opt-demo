...
df3_to_list <- function(dd) {
    ret <- alply(dd, 1, function(row) list(lat=as.vector(row[c(2,4,6)]), lon=as.vector(row[c(1,3,5)]), color='black'))
    names(ret) <- NULL
    ret
}
...
plot_with_triangles(df3_to_list(data.frame(t(three_loc_de$optim$bestmem))),
                    style='#!  function(feature) {
                        return { color: feature.properties.color }
                     } !#', zoomlevel=11)
