...
d_ply(sum_work_locs, 1, function(row) {
  with(row, map$marker(c(lat, lon), bindPopup = type))
})
...
map$setView(times_square, zoom = 10)
map$tileLayer(provider = 'Stamen.Toner')
d_ply(dat, 1, function(row) {
  with(row, if (!is.na(home_latitude[[1]])) map$marker(c(home_latitude[[1]], home_longitude[[1]])))
  with(row, if (!is.na(work_latitude[[1]])) map$marker(c(work_latitude[[1]], work_longitude[[1]])))
})
# TODO: Leaflet divIcon??
map
```
