...
mapping with the `ggmaps` and `rmaps` package (beta!), continuous optimization with `optim`,
...
mk_marker <- function(lat, lon, name) {
    list(type='Feature',
         properties=list(name=name, popupContent=name, color='red'),
         geometry=list(type='Point', coordinates=c(lon, lat)))
}
markers <- dlply(sum_work_locs, 1, function(row) with(row, mk_marker(lat, lon, type)))
...
map$setView(times_square, zoom = 10)
map$tileLayer(provider = 'Stamen.Toner')
d_ply(p2p_venue_costs, 1, function(row) {
  with(row, map$marker(c(lat, lon), bindPopup = name))
})
...
commute_lines <- alply(dat, 1, function(row) with(row, mk_commuteline(home_latitude[[1]], home_longitude[[1]],
                                                                work_latitude[[1]], work_longitude[[1]])))
names(commute_lines) <- NULL # so converter creates a list
map$geoJson(commute_lines, list(style=list(color="#ff7800", weight=5, opacity=0.65)))
...
map$setView(times_square, zoom = 10)
map$tileLayer(provider = 'Stamen.Toner')
qq <- transform(p2ls_venue_costs, address=name, lat=lat, lng=lon, lon=NULL, radius=50, color = 'red', fillColor = '#f03', fillOpacity = 0.5)
#map$circle2(qq)
# d_ply(p2ls_venue_costs, 1, function(row) {
#   with(row, map$circle(c(lat,lon)))    #arker(c(lat, lon), bindPopup = name))
# })
# TODO: color-code the icons!!
#map$circle(c(p2ls_venue_costs$lat[[1]], p2ls_venue_costs$lon[[1]]), radius=1000)
map$circle2(qq) # issues with the iterator in the circle2 code
map
```
...
Columbia's not great in either case.
...
grid <- expand.grid(lat=seq(from=times_square[[1]]-.5, to=times_square[[1]]+.5, length.out=100),
                        lon=seq(from=times_square[[2]]-.5, to=times_square[[2]]+.5, length.out=100))
grid <- adply(grid, 1, 
              function(rr) data.frame(cost=p2ls_cost(dat, rr$lon, rr$lat)))
grid$goodness <- 1-(grid$cost/max(grid$cost))
grid_json <- rjson::toJSON(rCharts::toJSONArray2(grid[,c('lat','lon','goodness')], json = F, names = F))
...
# grid_json = '[[40.7577,-73.9857,0.909874011663595]]'
# # Add javascript to modify underlying chart
# map$setTemplate(afterScript = sprintf("
# <script>
#   var addressPoints = %s
#   var heat = L.heatLayer(addressPoints, {radius: 13, blur: 1, gradient: {0.4: 'blue', 0.65: 'lime', 1: 'red'} }).addTo(map)           
# </script>
# ", grid_json
# ))
...
map <- Leaflet$new()
map$setView(c(single_p2ls_optim$y, single_p2ls_optim$x), zoom = 14)
map$tileLayer(provider = 'Stamen.Toner')
d_ply(p2p_venue_costs, 1, function(row) {
  with(row, map$marker(c(single_p2ls_optim$y, single_p2ls_optim$x), bindPopup = name))
})
# TODO: Leaflet divIcon??
map
